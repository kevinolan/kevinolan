 ...
 *
 * Ported from `speech-therapy-app/src/lib/fluency.ts` (pure TS, no deps).
 *
 * This implements the "recognition" half of the project's goal
 * ("controlled synthesis and recognition of stutter speech") without
 * requiring an ML model: it scans a transcript for surface stuttering
 * behaviors — part-word repetitions, sound prolongations, blocks, filler
 * words, unusually long pauses — and reports simple, encouraging metrics.
 *
 * Clinical note: this is a heuristic signal for self-awareness, NOT a
 * clinical diagnosis. ASHA emphasizes considering observable disfluency
 * together with internal reactions, avoidance, communication
...
function buildSummary(r: {
  ...
  if (r.wordCount === 0)
    return 'Say a few words while recording to see your fluency feedback here.';
  if (r.disfluencies === 0) {
    return `Nice smooth run — ${r.wordCount} words with no marked disfluencies. Keep this pace.`;
  }
  const parts: string[] = [];
  if (r.repetitions)
    parts.push(`${r.repetitions} repetition${r.repetitions > 1 ? 's' : ''}`);
  if (r.prolongations)
    parts.push(`${r.prolongations} prolongation${r.prolongations > 1 ? 's' : ''}`);
